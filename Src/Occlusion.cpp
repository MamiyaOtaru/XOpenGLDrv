...
void UXOpenGLRenderDevice::BuildStaticLightmapAtlas(const FString& AtlasPNG, const FString& AtlasMeta)
{
    ...
    // Choose atlas width:
    const INT MinAtlasWidth = 256; // tune if you like

    INT AtlasWidth = IdealSize;
    AtlasWidth = Max(AtlasWidth, MaxLMWidth);
    AtlasWidth = Max(AtlasWidth, MinAtlasWidth);

    // Compute required height = ceil(totalPixels / AtlasWidth)
    float RowsNeeded = float(TotalPixels) / float(AtlasWidth);
    INT AtlasHeight = appCeil(RowsNeeded);
    ...
    // Simple row-by-row packer (using padded sizes)
    INT CursorX   = 0;
    INT CursorY   = 0;
    INT RowHeight = 0;
    ...
    for (INT i = 0; i < PendingLightmaps.Num(); ++i)
    {
        ...
        {
            debugf(TEXT("XOpenGL: Atlas overflow — increase AtlasWidth or improve packing"));
            break;
        }
        ...
        // 1) Copy interior (offset by +1,+1 inside padded region)
        ...
        // 2) Duplicate top and bottom rows (RGB(A) from nearest interior)
        {
        ...
        // Store atlas UVs for the *interior* (skip the 1px padding)
        LM.AtlasX = DestX + 1;
        LM.AtlasY = DestY + 1;
...
void UXOpenGLRenderDevice::BuildPerSurfaceStaticLight(ULevel* Level, const FString& AtlasPNG, const FString& AtlasMeta)
{
    ...
    for (INT iSurf = 0; iSurf < Model->Surfs.Num(); ++iSurf)
    {
        ...
        INT W = Clamp(appRound(USize * Density), 8, 12);
        INT H = Clamp(appRound(VSize * Density), 8, 12);
