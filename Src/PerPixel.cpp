...
void ComputeStaticLightsForFacet(
    FSceneNode* Frame,
    ULevel* Level,
    INT iSurf,
    ...
    INT MaxStaticLights)
{
    ...
    // --- Cull static lights in world space ---
    for (INT i = 0; i < Level->Actors.Num(); ++i)
    {
        AActor* A = Level->Actors(i);
        if (!A || !IsStaticLight(A))
            continue;

        const FVector LightPos = A->Location;
        const float Radius = A->WorldLightRadius();

        // Quick sphere test
        float distToCentroid = (LightPos - C).Size();
        if (distToCentroid > Radius + facetRadius + 50.f)
            continue;

        // Project onto plane
        float planeDist = (LightPos - C) | N;
        FVector projected = LightPos - N * planeDist;

        // Inside test (fan triangulation)
        bool inside = false;
        const FVector& P0 = Verts(0);

        for (INT t = 1; t < Verts.Num() - 1; ++t)
        {
            const FVector& P1 = Verts(t);
            const FVector& P2 = Verts(t+1);

            FVector v0 = P1 - P0;
            FVector v1 = P2 - P0;
            FVector v2 = projected - P0;
        ...
        // If outside, find closest point on edges
        FVector closest = projected;
        if (!inside)
        {
            float minDistSq = FLT_MAX;

            for (INT e = 0; e < Verts.Num(); ++e)
            {
                const FVector& P1 = Verts(e);
                const FVector& P2 = Verts((e+1) % Verts.Num());

                FVector edge = P2 - P1;
                float lenSq = edge | edge;
                if (lenSq < 1e-6f)
                    continue;

                float t = ((projected - P1) | edge) / lenSq;
                t = Clamp(t, 0.f, 1.f);

                FVector cp = P1 + edge * t;
                float dSq = (projected - cp).SizeSquared();
    ...
    // Optional ranking
    if (OutLights.Num() > MaxStaticLights)
    {
        //TArray<AActor*> Reduced;
        //RankStaticLightsForFacet_WorldSpace(C, N, Verts, OutLights, Reduced, MaxStaticLights);
        //OutLights = Reduced;
    }
}
